...
export default class UserMongo {
  ...
  updateRole = async (userId, newUserRole) => {
    try {
      ...
      } else if (user.documents.length !== 3) {
  ...
  uploadPhotoPath = async (userId, path) => {
    try {
      ...
      logger.error(
        "ha ocurrido un error en la db al realizar la consulta: ",
        error
      );
      throw error;
    }
  ...
  deleteUser = async () => {
    try {
      ...
      const usersCartsIds = usuariosInactivos.map((user) =>
        user.carts.map((cart) => cart._id)
      );
